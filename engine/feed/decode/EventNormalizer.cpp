...
NormalizationResult EventNormalizer::normalize_json(
    ...
    if (json.is_object()) {
        result.events.push_back(normalize_one_object(packet, json));
        return result;
    }
    ...
    if (json.is_array()) {
        for (const auto& element : json.as_array()) {
            ...
            result.events.push_back(normalize_one_object(packet, element));
        }
...
NormalizationResult EventNormalizer::normalize_control(
    ...
    NormalizedEvent event = make_base_event(packet);
...
NormalizedEvent EventNormalizer::make_base_event(
    ...
    event.packet_id = packet.header.packet_id;
    event.recv_wall_ns = packet.header.recv_wall_ns;
    event.recv_monotonic_ns = packet.header.recv_monotonic_ns;
    event.source_id = packet.header.source_id;
...
NormalizedEvent EventNormalizer::normalize_one_object(
    ...
    if (raw_type == "book") {
        return normalize_book(packet, object, raw_type);
    }

    if (raw_type == "price_change") {
        return normalize_price_change(packet, object, raw_type);
    }

    if (raw_type == "best_bid_ask") {
        return normalize_best_bid_ask(packet, object, raw_type);
    }

    if (raw_type == "tick_size_change") {
        return normalize_tick_size_change(packet, object, raw_type);
    }

    if (raw_type == "new_market" || raw_type == "market_resolved") {
        return normalize_lifecycle(packet, object, raw_type);
    }

    if (raw_type == "last_trade_price") {
        return normalize_trade(packet, object, raw_type);
    }

    return normalize_unknown(packet, object, raw_type);
}
...
std::string to_string(NormalizedEventType type) {
    switch (type) {
...
std::string to_string(NormalizedSide side) {
    switch (side) {
