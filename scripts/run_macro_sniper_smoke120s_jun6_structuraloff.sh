#!/usr/bin/env bash
# LAB ONLY: disables structural dislocation guard (macro-max-local-oracle-divergence-tick=0)
# to force taker fills and validate reduction state machine. Never use on live capital.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

RUN_DIR="runs/market_maker_btc_above_64k_jun6_macro_sniper_smoke120s_structuraloff_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$RUN_DIR"
...
./build/run_market_maker_dashboard_live \
  ...
  --macro-max-local-oracle-divergence-tick 0 \
  --macro-divergence-cooldown-ms 250 \
