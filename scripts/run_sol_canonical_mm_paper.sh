...
RUN_ROOT="runs/sol_canonical_mm_paper_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$RUN_ROOT"
echo "$RUN_ROOT" > /tmp/polytope_last_sol_canonical_mm_paper_run_dir

DURATION_SECONDS="${DURATION_SECONDS:-14400}"
STARTING_CASH="${STARTING_CASH:-10000}"
SOL_VOL="${SOL_VOL:-0.90}"
...
COMMON_ARGS=(
  ...
  --canonical-quote-research-log
  --external-fair-tradable-lambda 0.20
  --enable-dynamic-inventory-targeter
  --enable-portfolio-touch-risk
...
echo "Dashboard: <run_dir>/<label>/dashboard.json"
